...
<div
  class="w-full flex justify-between flex-col mb-5 transition-all duration-300 sticky top-[3rem] pb-4 z-40 bg-gray-800"
>
  <h1
    class={` transition-all duration-700 whitespace-nowrap  logo font-bold ${
      !isInputFocused
        ? "mt-[0px] text-5xl"
        : "md:mt-[150px] mt-[100px] text-6xl md:text-8xl"
    }`}
  ...
  <div class="flex justify-between items-center w-full gap-6 mt-5">
    <div
      class={`relative w-full bg-gradient-to-r from-[#6EE7B7] via-[#3B82F6] to-[#9333EA] ${
        !isInputFocused ? "p-[2px]" : "p-[3px]"
      }`}
    >
      <input
        class={`w-full   transition-all border-1 border-solid outline-none bg-gray-800 text-gray-100  border-gray-900 duration-300 ease-in ${
          !isInputFocused ? " p-[0.5rem]" : "p-[1rem] "
        }`}
        type="text"
